...
class MwNfd(Application):
    ...
    def __init__(self, node, logLevel='NONE', csSize=65536,
        ...
        possibleConfPaths = ['/usr/local/etc/ndn/mw-nfd.conf.sample', '/usr/local/mw-etc/ndn/nfd.conf',
                             '/etc/ndn/mw-nfd.conf.sample', '/etc/ndn/mw-nfd.conf']
    ...
    def start(self):
        Application.start(self, 'mw-nfd --config {}'.format(self.confFile), logfile=self.logFile)
        Minindn.sleep(2)
